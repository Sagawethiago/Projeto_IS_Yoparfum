from django.http import HttpResponse
from django.shortcuts import render, redirect, get_object_or_404
from django.urls import reverse
from django.contrib import auth, messages
from django.urls import reverse_lazy
from rolepermissions.decorators import has_permission_decorator
from .models import Users
from django.contrib.auth.decorators import login_required


# View para a página de boas-vindas
@login_required
def welcome_page(request):
    return render(request, 'welcome_page.html')

# View para cadastrar produtos
#@has_permission_decorator('cadastrar_produto')
@login_required
def cadastrar_produto(request):
    # Lógica para cadastrar produtos
    return redirect(reverse_lazy('estoque:add_produto'))

# View para consultar estoque
#@has_permission_decorator('consultar_estoque')
@login_required
def consultar_estoque(request):
    # Lógica para consultar estoque
    return redirect(reverse_lazy('estoque:list_produtos'))

# View para cadastrar vendedor
@has_permission_decorator('cadastrar_vendedor')
def cadastrar_vendedor(request):
    if request.method == "GET":
        vendedores = Users.objects.filter(cargo="V")
        return render(request, 'cadastrar_vendedor.html', {'vendedores': vendedores})
    if request.method == "POST":
        nome = request.POST.get('nome')
        sobrenome = request.POST.get('sobrenome')
        email = request.POST.get('email')
        senha = request.POST.get('senha')

        user = Users.objects.filter(email=email)

        if user.exists():
            return HttpResponse('Email já existe')

        user = Users.objects.create_user(username=email,
                                         email=email,
                                         password=senha,
                                         first_name=nome,
                                         last_name=sobrenome,
                                         cargo="V")

        return HttpResponse('Conta criada')

# View para efetuar login
def login(request):
    if request.method == "GET":
        if request.user.is_authenticated:
            return redirect(reverse('welcome_page'))  # Redireciona para a página de boas-vindas se o usuário já estiver autenticado
        return render(request, 'login.html')
    elif request.method == "POST":
        login = request.POST.get('email')
        senha = request.POST.get('senha')

        user = auth.authenticate(username=login, password=senha)

        if not user:
            return HttpResponse('Usuário inválido')

        auth.login(request, user)
        return redirect(reverse('welcome_page'))  # Redireciona para a página de boas-vindas após o login

# View para efetuar logout
def logout(request):
    request.session.flush()
    return redirect(reverse('login'))

# View para excluir usuário
#@has_permission_decorator('cadastrar_vendedor')
def excluir_usuario(request, id):
    vendedor = get_object_or_404(Users, id=id)
    vendedor.delete()
    messages.add_message(request, messages.SUCCESS, 'Vendedor excluído com sucesso')
    return redirect(reverse('cadastrar_vendedor'))
#dashboard e api
def dashboard(request):
    # Lógica para exibir o dashboard
    return render(request, 'welcome_page.html')

def api_produtos(request):
    # Lógica para a API
    return redirect(reverse_lazy('estoque:api_produtos'))
    #return render(request, 'welcome_page.html')

